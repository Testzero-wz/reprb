...
def test():
    ...
    # iterable
    iterable_list = [b"\0", b"\x11"]

    def gen():
        for i in iterable_list:
            yield i

    dump(gen(), tmp_dump_file)
...
def benchmark():
    ...
    bench("built-in eval", eval, repr(bytes_msg), bench_times)
    bench("urllib.parse.quote_from_bytes", quote_from_bytes, bytes_msg, bench_times)
    bench(
        "urllib.parse.unquote_to_bytes",
        unquote_to_bytes,
        quote_from_bytes(bytes_msg),
        bench_times,
    )
    bench("reprb/dumpb", reprb, bytes_msg, bench_times)
    bench("evalb/loadb", evalb, reprb(bytes_msg), bench_times)
